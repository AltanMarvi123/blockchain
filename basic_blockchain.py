...
class Blockchain: 
    ...
    def proof_of_work(self, previous_proof): 
        ...
        while check_proof is False:
            self.hash_operation = hashlib.sha256(str(new_proof**2 - previous_proof**2).encode()).hexdigest()
            if self.hash_operation[:self.difficulty] == '0'*self.difficulty:
                check_proof = True
                self.difficulty += 1
                print(f"Proof found: {self.hash_operation}")
            else: 
                new_proof += 1
        return new_proof
